...
// Predefined packet arrays removed - now handled by Attack class
...
// Attack object for multi-vector deauth attack
Attack attack;
...
void loop() {
  ...
  if (jammerActive) {
    // Use the Attack class to handle packet sending
    attack.update();
    
    // Update local packet count for display
    packetsSent = attack.getPacketsSent();
  }
}
...
void startJammer() {
  ...
  // Start attack using Attack class
  attack.start(targetMAC, targetChannel);
